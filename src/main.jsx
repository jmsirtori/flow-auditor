...
function App() {
  ...
  const sendMessage = async (text) => {
    ...
    setLoading(true);
    try {
      const result = await runAuditLoop(newMessages.map(m => ({ role: m.role, content: m.content })), apiKey, setStatus);
  ...
  if (showSetup) return (
    <div style={{ fontFamily: "'IBM Plex Mono',monospace", background: "#060a10", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <div style={{ maxWidth: 420, width: "100%", background: "#0d1117", border: "1px solid #30363d", borderRadius: 12, padding: 32 }}>
        ...
        <div style={{ fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700, fontSize: 18, color: "#e6edf3", textAlign: "center", marginBottom: 8 }}>Digital Flow & SEO Auditor</div>
        ...
        />
        <button onClick={saveApiKey} style={{ width: "100%", background: "#f0883e", border: "none", borderRadius: 8, padding: "12px", color: "#060a10", fontFamily: "inherit", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
          COMENZAR →
        </button>
        <div style={{ fontSize: 10, color: "#30363d", textAlign: "center", marginTop: 14, lineHeight: 1.6 }}>
    ...
    <div style={{ fontFamily: "'IBM Plex Mono',monospace", background: "#060a10", minHeight: "100vh", color: "#c9d1d9", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ padding: "14px 24px", borderBottom: "1px solid #161b22", display: "flex", alignItems: "center", gap: 12, background: "#0d1117", position: "sticky", top: 0, zIndex: 50 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 10, color: "#484f58", display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}><span className="pulse-dot" /> Flow Company · 2026</div>
        </div>
      ...
      <div style={{ padding: "12px 20px 16px", borderTop: "1px solid #161b22", background: "#0d1117", maxWidth: 820, width: "100%", margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "flex-end", background: "#161b22", border: "1px solid #30363d", borderRadius: 10, padding: "12px 14px" }}>
          <textarea ref={textareaRef} rows={2} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKey} placeholder="Ej: 🔍 Audita mi negocio: Flow Company - https://flowcompany.mx" />
          <button className="send-btn" onClick={() => sendMessage()} disabled={loading || !input.trim()}>{loading ? "AUDITANDO..." : "AUDITAR →"}</button>
        </div>
      </div>
    </div>
  );
