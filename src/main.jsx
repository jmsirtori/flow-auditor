// Ignitia · SEO Auditor v9.0 — Stitch UI + Full Logic
...
function getMasterPrompt(lang) {
  const isEN = lang === "en";
  return `ROL: You are the "Digital Growth Auditor" of Ignitia. IMPORTANT: ${isEN?"Respond ENTIRELY in English.":"Responde COMPLETAMENTE en español."}

PHILOSOPHY: Visibility is a connected system. If one point fails, the customer is lost.

I. AUDIT PROTOCOL:
1. Brand Consistency (NAP): Compare Name, Address, Phone on Web, Maps, Social.
2. Deep SEO: Extract keywords, simulate search, compare Top 3 competitors.
3. Friction Diagnosis: Find H1. JavaScript-dependent content = "Closed Shutter for Google".
4. Connected Flow: Are Social Media links strategic?
...
- ${isEN?"Sales Hook: key weak point.":"Gancho de Venta: punto débil clave."}

## 📄 ${isEN?"CLIENT REPORT":"REPORTE PARA EL CLIENTE"}
### ⚡ ${isEN?"Digital Health Diagnosis":"Diagnóstico de Salud Digital"}
${isEN?"Score 1-10 Friction (1=easy, 10=impossible to find).":"Calificación 1-10 Fricción."}
...
${isEN?"If not found: 'Could not verify, please review manually'. Don't say beautiful if not functional.":"Si no encuentras: 'No pude verificar, favor revisar manualmente'. No digas bonita si no es funcional."}`;
}
...
function TopBar({setView}){
  return(<nav style={{position:"fixed",top:0,left:0,right:0,height:64,zIndex:50,background:"rgba(23,28,34,0.85)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(255,69,0,0.15)",boxShadow:"0 0 40px rgba(255,69,0,0.06)",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 24px"}}>
    <div onClick={()=>setView("dashboard")} style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}}><div style={{width:32,height:32,background:"linear-gradient(135deg,#FF4500,#c43300)",borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>🔥</div><span style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:16,color:"#dfe2ec",textTransform:"uppercase",letterSpacing:"0.1em"}}>Ignitia</span></div>
    <div style={{display:"flex",alignItems:"center",gap:16}}><div style={{width:8,height:8,borderRadius:"50%",background:"#3fb950",boxShadow:"0 0 8px rgba(63,185,80,0.5)"}}/><span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58",textTransform:"uppercase",letterSpacing:"0.1em"}}>IGNITIA v9.0-STABLE</span></div>
  </nav>);
}
  ...
  return(<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:24,background:"#060a10",position:"relative",overflow:"hidden"}}>
    ...
    <div style={{position:"relative",zIndex:10,width:"100%",maxWidth:420}}>
      ...
      <div style={{textAlign:"center",marginBottom:32}}>
        ...
        <div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:36,color:"#fff",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:6}}>Ignitia</div>
        <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:10,color:"#484f58",textTransform:"uppercase",letterSpacing:"0.2em"}}>{t("appTagline")}</div>
      </div>
      <div style={{background:"#0f141a",border:"1px solid rgba(255,255,255,0.05)",borderRadius:12,padding:36,boxShadow:"0 0 40px rgba(255,69,0,0.05)"}}>
        <div style={{marginBottom:28}}><div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:18,color:"#fff",textTransform:"uppercase",marginBottom:4}}>{t("login")}</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#484f58"}}>{t("loginSub")}</div></div>
        {[{label:t("emailLabel"),type:"email",value:email,set:setEmail,placeholder:t("emailPlaceholder")},{label:t("passwordLabel"),type:"password",value:password,set:setPassword,placeholder:t("passwordPlaceholder")}].map(f=>(<div key={f.label} style={{marginBottom:24}}><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,textTransform:"uppercase",letterSpacing:"0.2em",color:"#484f58",marginBottom:8}}>{f.label}</div><input type={f.type} value={f.value} onChange={e=>f.set(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleLogin()} placeholder={f.placeholder} style={{width:"100%",background:"transparent",border:"none",borderBottom:"1px solid rgba(255,255,255,0.1)",padding:"10px 0",color:"#fff",fontFamily:"'IBM Plex Mono',monospace",fontSize:13,outline:"none",boxSizing:"border-box"}}/></div>))}
        {error&&<div style={{background:"rgba(232,75,74,0.1)",border:"1px solid rgba(232,75,74,0.3)",borderRadius:4,padding:"10px 14px",color:"#E24B4A",fontSize:11,fontFamily:"'IBM Plex Mono',monospace",marginBottom:20}}>{error}</div>}
        <button onClick={handleLogin} disabled={loading} style={{width:"100%",background:loading?"#7a2200":"#FF4500",border:"none",borderRadius:8,padding:"14px",color:"#fff",fontFamily:"'Space Grotesk',sans-serif",fontSize:13,fontWeight:700,cursor:loading?"not-allowed":"pointer",textTransform:"uppercase",letterSpacing:"0.2em"}}>{loading?t("loginLoading"):t("loginBtn")}</button>
        <div style={{marginTop:20,textAlign:"center",fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58",textTransform:"uppercase",letterSpacing:"0.1em"}}>{t("loginRestricted")}</div>
      </div>
      <div style={{marginTop:28,display:"flex",justifyContent:"space-between",padding:"0 4px"}}><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{width:8,height:8,borderRadius:"50%",background:"#3fb950",display:"inline-block",boxShadow:"0 0 8px rgba(63,185,80,0.5)"}}/><span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58",textTransform:"uppercase"}}>System Status: Operational</span></div><span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58",textTransform:"uppercase"}}>IGNITIA v9.0</span></div>
    </div>
  </div>);
}
...
function DashboardView({onNewAudit,onSelectClient}){
  ...
  useEffect(()=>{
    ...
    window.ignitiaChart=new window.Chart(ctx,{type:"bar",data:{labels:chartClients.map(c=>c.name.length>10?c.name.slice(0,10)+"…":c.name),datasets:[{data:chartClients.map(c=>c.lastScore),backgroundColor:chartClients.map(c=>c.lastScore<=4?"#E24B4A":c.lastScore<=7?"#d29922":"#3fb950"),borderRadius:0}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{min:0,max:10,ticks:{stepSize:2,color:"#484f58",font:{family:"'IBM Plex Mono',monospace",size:10}},grid:{color:"rgba(255,255,255,0.03)"}},x:{ticks:{color:"#484f58",font:{family:"'IBM Plex Mono',monospace",size:9},autoSkip:false,maxRotation:45},grid:{display:false}}}}});
  },[clients.length]);
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:36}}>
    <header><div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:36,color:"#fff",textTransform:"uppercase",letterSpacing:"-0.02em",marginBottom:4}}>{t("dashboard")}</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58",textTransform:"uppercase",letterSpacing:"0.2em"}}>V9.0-STABLE // ACTIVE SESSION</div></header>

    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:16}}>
      {stats.map((s,i)=>(<div key={i} style={{background:"#171c22",borderLeft:`4px solid ${s.color}`,padding:"18px 22px"}}><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,textTransform:"uppercase",letterSpacing:"0.15em",color:"#484f58",marginBottom:12}}>{s.label}</div><div style={{display:"flex",alignItems:"flex-end",gap:10}}><span style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:32,color:s.color,lineHeight:1}}>{s.value}</span>{s.change&&<div style={{display:"flex",alignItems:"center",gap:5,marginBottom:3}}>{s.pulse&&<span style={{width:6,height:6,borderRadius:"50%",background:s.color,display:"inline-block"}}/>}<span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:8,textTransform:"uppercase",color:s.pulse?s.color:"#484f58",fontWeight:s.pulse?700:400}}>{s.change}</span></div>}</div></div>))}
    </div>

    {clients.length===0?(<div style={{background:"#171c22",border:"1px dashed #31353c",borderRadius:8,padding:"56px 24px",textAlign:"center"}}><div style={{fontSize:36,marginBottom:14}}>🔥</div><div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:16,color:"#fff",textTransform:"uppercase",marginBottom:6}}>{t("emptyTitle")}</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:12,color:"#484f58",lineHeight:1.7,marginBottom:24,maxWidth:360,margin:"0 auto 24px"}}>{t("emptyDesc")}</div><button onClick={onNewAudit} style={{background:"#FF4500",border:"none",color:"#fff",padding:"12px 28px",cursor:"pointer",fontFamily:"'Space Grotesk',sans-serif",fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.2em"}}>{t("startAudit")}</button></div>
    ):(<div style={{display:"flex",flexDirection:"column",gap:20}}>
      {chartClients.length>0&&(<div style={{background:"#171c22",padding:28,borderRadius:4}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:28}}><div><div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:16,textTransform:"uppercase"}}>{t("frictionChart")}</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58",textTransform:"uppercase",letterSpacing:"0.15em",marginTop:4}}>Cross-segment analysis</div></div><div style={{display:"flex",gap:14}}>{[["#E24B4A","High"],["#d29922","Med"],["#3fb950","Low"]].map(([color,label])=>(<div key={label} style={{display:"flex",alignItems:"center",gap:5}}><span style={{width:7,height:7,borderRadius:"50%",background:color,display:"inline-block"}}/><span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em",color:"#484f58"}}>{label}</span></div>))}</div></div>
        <div style={{position:"relative",height:180}}><canvas id="dashChart"/></div>
      </div>)}
      <div style={{background:"#171c22",borderRadius:4,overflow:"hidden"}}>
        <div style={{padding:"20px 28px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"1px solid rgba(255,255,255,0.05)"}}><div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:18,textTransform:"uppercase"}}>{t("allClients")}</div><button onClick={onNewAudit} style={{background:"#FF4500",border:"none",color:"#fff",padding:"8px 18px",cursor:"pointer",fontFamily:"'Space Grotesk',sans-serif",fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.15em"}}>+ {t("newAudit").replace("+ ","")}</button></div>
        <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontFamily:"'IBM Plex Mono',monospace"}}>
          <thead><tr style={{background:"#0f141a"}}>{[t("client"),t("sector"),t("score"),t("audits"),t("lastAudit"),t("operations")].map((h,i)=>(<th key={h} style={{padding:"12px 20px",textAlign:i>=2?"center":"left",fontFamily:"'IBM Plex Mono',monospace",fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.15em",color:"#484f58"}}>{h}</th>))}</tr></thead>
          <tbody>{clients.map(client=>{const audits=getClientAudits(client.id);return(<tr key={client.id} style={{borderBottom:"1px solid rgba(255,255,255,0.03)",cursor:"pointer"}} onMouseOver={e=>e.currentTarget.style.background="#31353c20"} onMouseOut={e=>e.currentTarget.style.background=""}><td style={{padding:"18px 20px"}}><div style={{fontWeight:700,fontSize:13,color:"#dfe2ec",marginBottom:2}}>{client.name}</div><div style={{fontSize:9,color:"#484f58",textTransform:"uppercase"}}>{client.url?.replace("https://","")}</div></td><td style={{padding:"18px 20px"}}><span style={{background:"#31353c",padding:"3px 10px",fontSize:9,color:"#484f58",textTransform:"uppercase"}}>{client.sector}</span></td><td style={{padding:"18px 20px",textAlign:"center"}}>{client.lastScore?<ScoreBadge score={client.lastScore}/>:<span style={{color:"#31353c",fontSize:10}}>{t("noScore")}</span>}</td><td style={{padding:"18px 20px",textAlign:"center",fontWeight:700,fontSize:14,color:"#dfe2ec"}}>{String(audits.length).padStart(2,"0")}</td><td style={{padding:"18px 20px",textAlign:"center",fontSize:10,color:"#484f58",textTransform:"uppercase"}}>{client.lastAudit||"—"}</td><td style={{padding:"18px 20px",textAlign:"right"}}><button onClick={()=>onSelectClient(client)} style={{background:"none",border:"none",color:"#484f58",cursor:"pointer",fontSize:11,textTransform:"uppercase",letterSpacing:"0.1em",fontFamily:"'IBM Plex Mono',monospace",fontWeight:700}} onMouseOver={e=>e.currentTarget.style.color="#FF4500"} onMouseOut={e=>e.currentTarget.style.color="#484f58"}>{t("viewBtn")}</button></td></tr>);})}</tbody>
        </table></div>
        <div style={{padding:"12px 28px",background:"#0f141a",borderTop:"1px solid rgba(255,255,255,0.03)"}}><span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.15em",color:"#484f58"}}>{t("showingOf")} {clients.length} {t("registered")}</span></div>
      </div>
    </div>)}
  </div>);
}
...
function SaveClientModal({onSave,onSkip,defaultName="",defaultUrl=""}){
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
    <div style={{background:"#0f141a",border:"1px solid rgba(255,255,255,0.05)",borderRadius:8,padding:32,maxWidth:400,width:"100%"}}>
      <div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:18,color:"#fff",textTransform:"uppercase",marginBottom:4}}>{t("saveClient")}</div>
      <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#484f58",marginBottom:24}}>{t("saveClientDesc")}</div>
      {[{label:t("nameLabel"),value:name,set:setName,placeholder:"Ej: Paseo Interlomas"},{label:t("urlLabel"),value:url,set:setUrl,placeholder:"Ej: https://paseointerlomas.mx"}].map(f=>(<div key={f.label} style={{marginBottom:18}}><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,textTransform:"uppercase",letterSpacing:"0.2em",color:"#484f58",marginBottom:7}}>{f.label}</div><input value={f.value} onChange={e=>f.set(e.target.value)} placeholder={f.placeholder} style={{width:"100%",background:"transparent",border:"none",borderBottom:"1px solid rgba(255,255,255,0.1)",padding:"8px 0",color:"#fff",fontFamily:"'IBM Plex Mono',monospace",fontSize:12,outline:"none",boxSizing:"border-box"}}/></div>))}
      <div style={{marginBottom:22}}><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,textTransform:"uppercase",letterSpacing:"0.2em",color:"#484f58",marginBottom:7}}>{t("sectorLabel")}</div><select value={sector} onChange={e=>setSector(e.target.value)} style={{width:"100%",background:"#171c22",border:"1px solid #31353c",padding:"8px 12px",color:"#dfe2ec",fontFamily:"'IBM Plex Mono',monospace",fontSize:11,outline:"none"}}>{SECTORS[lang].map(s=><option key={s} value={s}>{s}</option>)}</select></div>
      <div style={{display:"flex",gap:10}}><button onClick={onSkip} style={{flex:1,background:"transparent",border:"1px solid #31353c",color:"#484f58",padding:"10px",cursor:"pointer",fontFamily:"'IBM Plex Mono',monospace",fontSize:11,textTransform:"uppercase"}}>{t("skipBtn")}</button><button onClick={()=>name.trim()&&onSave({name:name.trim(),url:url.trim(),sector})} style={{flex:1,background:"#FF4500",border:"none",color:"#fff",padding:"10px",cursor:"pointer",fontFamily:"'Space Grotesk',sans-serif",fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em"}}>{t("saveBtn")}</button></div>
    </div>
  </div>);
}
...
function ReauditModal({client,onCompare,onFresh,onCancel}){
  ...
  return(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
    <div style={{background:"#0f141a",border:"1px solid rgba(255,255,255,0.05)",borderRadius:8,padding:32,maxWidth:400,width:"100%"}}>
      <div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:18,color:"#fff",textTransform:"uppercase",marginBottom:4}}>{t("reauditTitle")}: {client.name}</div>
      <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#484f58",marginBottom:24}}>{audits.length} {t("prevAuditsLabelPlural")}</div>
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {audits.length>0&&(<button onClick={onCompare} style={{background:"rgba(162,201,255,0.05)",border:"1px solid rgba(162,201,255,0.2)",color:"#a2c9ff",padding:14,cursor:"pointer",fontFamily:"'IBM Plex Mono',monospace",fontSize:11,fontWeight:700,textAlign:"left"}}>{t("compareBtn")}<div style={{fontSize:10,color:"#484f58",marginTop:4,fontWeight:400}}>{t("compareDesc")}</div></button>)}
        <button onClick={onFresh} style={{background:"rgba(255,69,0,0.05)",border:"1px solid rgba(255,69,0,0.2)",color:"#FF4500",padding:14,cursor:"pointer",fontFamily:"'IBM Plex Mono',monospace",fontSize:11,fontWeight:700,textAlign:"left"}}>{t("freshBtn")}<div style={{fontSize:10,color:"#484f58",marginTop:4,fontWeight:400}}>{t("freshDesc")}</div></button>
        <button onClick={onCancel} style={{background:"transparent",border:"1px solid #31353c",color:"#484f58",padding:10,cursor:"pointer",fontFamily:"'IBM Plex Mono',monospace",fontSize:11,textTransform:"uppercase"}}>{t("cancelBtn")}</button>
      </div>
    </div>
  </div>);
}
...
function HistoryView({onLoad}){
  ...
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:36}}>
      <div><div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:32,color:"#fff",textTransform:"uppercase",marginBottom:4}}>{t("historyTitle")}</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:12,color:"#484f58"}}>{history.length} {t("historyAudits")}</div></div>
      <div style={{position:"relative"}}><span style={{position:"absolute",left:12,top:"50%",transform:"translateY(-50%)",color:"#484f58",fontSize:12}}>🔍</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder={t("searchPlaceholder")} style={{background:"#171c22",border:"none",borderBottom:"2px solid rgba(255,255,255,0.1)",padding:"10px 10px 10px 34px",color:"#fff",fontFamily:"'IBM Plex Mono',monospace",fontSize:11,outline:"none",width:260}}/></div>
    </div>
    <div style={{display:"flex",gap:10,marginBottom:32,flexWrap:"wrap"}}>{[["todos",t("filterAll")],["mes",t("filterMonth")],["cliente",t("filterClient")],["critico",t("filterCritical")]].map(([val,lbl])=>(<button key={val} onClick={()=>setFilter(val)} style={{padding:"5px 18px",borderRadius:20,border:`1px solid ${filter===val?"#FF4500":"rgba(255,255,255,0.1)"}`,background:filter===val?"rgba(255,69,0,0.05)":"transparent",color:filter===val?"#FF4500":"#484f58",fontFamily:"'IBM Plex Mono',monospace",fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em",cursor:"pointer"}}>{lbl}</button>))}</div>
    {filtered.length===0?(<div style={{textAlign:"center",padding:"56px 0",color:"#484f58"}}><div style={{fontSize:36,marginBottom:14}}>📭</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,textTransform:"uppercase"}}>{search?t("emptySearch"):t("emptyHistory")}</div></div>
      ...
      <div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:3}}>{t("executionDate")}</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11}}>{entry.date}</div></div>
      <div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:3}}>Client</div>{getClientName(entry.clientId)?<span style={{background:"#31353c",padding:"2px 8px",fontFamily:"'IBM Plex Mono',monospace",fontSize:10,color:"#FF4500",fontWeight:700,textTransform:"uppercase"}}>{getClientName(entry.clientId)}</span>:<span style={{color:"#484f58",fontSize:10}}>—</span>}</div>
      <div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:3}}>{t("performance")}</div>{entry.score?<ScoreBadge score={entry.score}/>:<span style={{color:"#484f58",fontSize:10}}>—</span>}</div>
      <div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:3}}>{t("queryContext")}</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"rgba(255,255,255,0.5)",fontStyle:"italic",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>"{entry.query}"</div></div>
      <div style={{display:"flex",gap:14,alignItems:"center"}}><button onClick={()=>onLoad(entry)} style={{background:"none",border:"none",color:"#FF4500",fontFamily:"'IBM Plex Mono',monospace",fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.1em",cursor:"pointer"}}>{t("viewBtn")}</button><button onClick={()=>handleDelete(entry.id)} style={{background:"none",border:"none",color:"#484f58",cursor:"pointer",fontSize:13}} onMouseOver={e=>e.currentTarget.style.color="#E24B4A"} onMouseOut={e=>e.currentTarget.style.color="#484f58"}>🗑</button></div>
    </div>))}</div>)}
  </div>);
}
...
function App(){
  ...
  const handleCompare=()=>{
    ...
    const context=`PREVIOUS AUDIT CONTEXT (${last.date}):\n${last.result.slice(0,1500)}\n\nSPECIAL INSTRUCTION: Compare with previous audit and add comparison section at the end.`;
  ...
  const globalStyles=`
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    body{background:#060a10;color:#dfe2ec;font-family:'IBM Plex Mono',monospace}
  ...
  if(authLoading)return(<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#060a10"}}><style>{globalStyles}</style><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#484f58",textTransform:"uppercase",letterSpacing:"0.2em"}}>{t("loading")}</div></div>);
    ...
    {!session?<LoginScreen/>:(
      <div style={{minHeight:"100vh",background:"#060a10"}}>
        ...
        <main style={{marginLeft:240,paddingTop:64,minHeight:"100vh"}}>
          <div style={{padding:"36px 36px 0",maxWidth:1100}}>
            ...
            {view==="chat"&&(<div style={{display:"flex",flexDirection:"column",height:"calc(100vh - 100px)"}}>
              ...
              {activeClient&&(<div style={{padding:"10px 0",borderBottom:"1px solid rgba(255,255,255,0.05)",marginBottom:16,display:"flex",alignItems:"center",gap:12}}><span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58",textTransform:"uppercase",letterSpacing:"0.2em"}}>Active_Audit:</span><span style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:14,color:"#FF4500",textTransform:"uppercase"}}>{activeClient.name}</span>{activeClient.url&&<span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58"}}>{activeClient.url.replace("https://","")}</span>}<button onClick={()=>{setActiveClient(null);setMessages([]);}} style={{marginLeft:"auto",background:"transparent",border:"1px solid rgba(255,69,0,0.3)",color:"#FF4500",padding:"3px 10px",borderRadius:4,cursor:"pointer",fontFamily:"'IBM Plex Mono',monospace",fontSize:9,textTransform:"uppercase"}}>{t("cleanBtn")}</button></div>)}
              <div style={{flex:1,overflowY:"auto",paddingBottom:130}}>
                {messages.length===0&&!loading&&(<div style={{paddingTop:32}}>
                  <div style={{marginBottom:28}}><div style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:28,color:"#fff",textTransform:"uppercase",letterSpacing:"-0.01em",marginBottom:4}}>{activeClient?`Active_Audit: ${activeClient.name}`:"Ignitia SEO Console"}</div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58",textTransform:"uppercase",letterSpacing:"0.2em"}}>{activeClient?`Status: Online // ${activeClient.sector}`:"Status: System Online // AI Engine Ready"}</div></div>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:24}}>
                  {messages.map((msg,i)=>(<div key={i}>
                    {msg.role==="user"?(<div style={{display:"flex",justifyContent:"flex-end",gap:12}}>
                      <div style={{maxWidth:"60%",textAlign:"right"}}><div style={{background:"#31353c",padding:"12px 16px",borderRadius:"10px 10px 0 10px"}}><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:12,lineHeight:1.65,color:"#dfe2ec",whiteSpace:"pre-wrap"}}>{msg.content}</div></div><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58",textTransform:"uppercase",letterSpacing:"0.1em",marginTop:5}}>{t("youLabel")}</div></div>
                      ...
                      <div style={{display:"flex",gap:12}}>
                        ...
                        <div style={{flex:1}}>
                          <div style={{marginBottom:10,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}><span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#FF4500",textTransform:"uppercase",letterSpacing:"0.15em",fontWeight:700}}>Ignitia AI // {t("reportLabel")}</span>{extractScore(msg.content)&&<ScoreBadge score={extractScore(msg.content)}/>}<span style={{marginLeft:"auto",fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58"}}>{t("saved")}</span></div>
                          {parseMarkdownSections(msg.content).map((section,si)=>{const s=getStyle(section.title);return(<div key={si} style={{background:s.bg,border:`1px solid ${s.border}`,borderRadius:8,padding:"18px 22px",marginBottom:10}}>
                            {section.title&&(<div style={{marginBottom:12}}><span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:8,fontWeight:700,letterSpacing:"0.2em",background:`${s.accent}20`,color:s.accent,padding:"3px 10px",textTransform:"uppercase"}}>{s.tag}</span><div style={{fontFamily:"'Space Grotesk',sans-serif",fontSize:14,fontWeight:700,color:s.accent,marginTop:8,textTransform:"uppercase",letterSpacing:"-0.01em"}}>{section.title}</div></div>)}
                            <div dangerouslySetInnerHTML={{__html:renderMd(section.content.join("\n"))}}/>
                          </div>);})}
                          <div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#484f58",textTransform:"uppercase",letterSpacing:"0.1em",marginTop:5}}>Ignitia AI // Audit_Agent</div>
                        </div>
                      </div>
                    )}
                  </div>))}
                  {loading&&(<div style={{display:"flex",gap:12}}>
                    ...
                    <div style={{flex:1,background:"#171c22",border:"1px solid rgba(255,69,0,0.1)",borderRadius:8,padding:"18px 22px"}}><div style={{height:2,background:"linear-gradient(90deg,#FF4500,#ffb5a0,#FF4500)",backgroundSize:"200%",animation:"shimmer 2s linear infinite",marginBottom:14,borderRadius:2}}/><div style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:11,color:"#FF4500",marginBottom:8}}>{getStatusText(statusKey)}</div><div style={{display:"flex",gap:14,flexWrap:"wrap"}}>{[t("tracking"),t("verifying"),t("keywords"),t("comparing")].map((txt,i)=>(<span key={i} style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:9,color:"#31353c",textTransform:"uppercase",letterSpacing:"0.1em",animation:`blink ${1+i*0.5}s step-end infinite`}}>{txt}...</span>))}</div></div>
                  </div>)}
                  <div ref={bottomRef}/>
                </div>
              </div>
              <div style={{position:"fixed",bottom:0,left:240,right:0,padding:"16px 36px 20px",background:"linear-gradient(to top, #060a10 60%, transparent)"}}>
                <div style={{maxWidth:1064,margin:"0 auto",background:"#171c22",border:"1px solid rgba(255,255,255,0.05)",borderRadius:12,boxShadow:"0 -4px 40px rgba(0,0,0,0.3)",padding:"8px 8px 8px 20px",display:"flex",alignItems:"center",gap:12}}>
                  ...
                  <textarea ref={textareaRef} rows={2} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&!e.shiftKey){e.preventDefault();sendMessage();}}} placeholder={activeClient?`${t("clientPlaceholder")} ${activeClient.name}...`:t("inputPlaceholder")} style={{flex:1,background:"transparent",border:"none",outline:"none",color:"#dfe2ec",fontFamily:"'IBM Plex Mono',monospace",fontSize:12,resize:"none",lineHeight:1.6,textTransform:"uppercase"}}/>
                  <button onClick={()=>sendMessage()} disabled={loading||!input.trim()} style={{background:loading?"#7a2200":"#FF4500",border:"none",color:"#fff",padding:"10px 22px",cursor:loading||!input.trim()?"not-allowed":"pointer",fontFamily:"'Space Grotesk',sans-serif",fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.15em",borderRadius:8,opacity:!input.trim()?0.3:1,flexShrink:0,transition:"all .2s"}}>{loading?t("auditingBtn"):`${t("auditBtn")} ⚡`}</button>
                </div>
                <div style={{textAlign:"center",marginTop:6}}><span style={{fontFamily:"'IBM Plex Mono',monospace",fontSize:8,color:"#31353c",textTransform:"uppercase",letterSpacing:"0.3em"}}>Console Secure // Encrypted Connection Enabled</span></div>
              </div>
            </div>)}
          </div>
        </main>
